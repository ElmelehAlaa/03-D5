...
*/
// const me = { name: "Alaa", surname: "Elmeleh", Age: 24 };
// console.log(me);
...
const deleteOne = function (string, boolean) {
  if (boolean === true) {
    return console.log(string.slice(1, string.lenght));
  } else {
    return console.log(string.slice(0, -1));
  }
};
deleteOne("Epicode is awesome", false);
/* ESERCIZIO 5
...
const onlyLetters = function (string) {
  return console.log(string.replace(/[0-9]/g, "")); // replace() in javascript sostituisce solo il primo valore che matcha , aggiungere /g vorra' dire che tutti i valori nella stringa verranno sostituiti.
};

onlyLetters("i have 1 dog and 2 cats");
/* ESERCIZIO 6
...
isThisAnEmail("alaa.elmeleh@outlook.it");
/* ESERCIZIO 7
  Scrivi una funzione chiamata "whatDayIsIt" che ritorna il giorno della settimana corrente.
*/
const day = new Date("July 14 , 2023 12:08:30");
const whatDayIsIt = function () {
  return console.log(day.getDay()); // inizia da domenica 0
};
whatDayIsIt();
/* ESERCIZIO 8
...
*/
const myBirthday = new Date("February 24,1999");

const isTodayMyBirtday = function (getDate) {
  if (myBirthday === getDate()) {
    return console.log("True è il mio compleanno!");
  } else {
    console.log("False! non e' il mio compleanno");
  }
};
//isTodayMyBirtday(getDate());
...
const deleteProp = function (object, string) {
  ...
  return console.log(delete object[string]);
};
//deleteProp({ oggetto }, (oggetto = { colore }));
...
const newestMovie = function (array) {
  ...
  array.forEach((movie) => {
    let filmYear = parseInt(movie.Year);
    if (filmYear > result.Year) {
      result = movie;
    }
  });
  return result;
...
const onlyTheYears = function (array) {
  return array.map((elem) => parseInt(elem.Year));
};
...
const searchByTitle = function (string) {
  return movies.find((elem) => elem.Title === string);
};
...
const searchAndDivide = function (str) {
  const titleYouSearch = {
    match: [],
    unmatch: [],
  };
  for (let i = 0; i < movies.length; i++)
    if (movies[i].Title.toLowerCase().includes(str.toLowerCase())) {
      titleYouSearch.match.push(movies[i]);
    } else {
      titleYouSearch.unmatch.push(movies[i]);
    }
...
const backgroundColor = function () {
  let linkBackground = document.querySelectorAll("a");
  linkBackground = Array.from(linkBackground);
  linkBackground.forEach((singleLink) => (singleLink.style.backgroundColor = "red"));
};
